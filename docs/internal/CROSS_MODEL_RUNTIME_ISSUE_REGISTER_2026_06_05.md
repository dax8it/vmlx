...
- Qwen36 27B MXFP8 TP4 relaunch proof, 2026-06-06: stale Pod1 Qwen36 serves/workers were stopped, a fresh resident run `qwen36-mxfp8-pod1-tp4-codexlive2-20260606T160731Z` was launched on Max2/ADLab Pod1, and a separate gateway was started on Max2 `127.0.0.1:8125`. Local summary artifact `build/current-qwen36-27b-tp4-codexlive2-strict-proof-20260606.json` records the strict proof verdict as `FAIL`, with full proof JSON on Max2 at `/tmp/adlab-qwen36-27b-mxfp8-tp4-codexlive2-qwen36-mxfp8-pod1-tp4-codexlive2-20260606T160731Z-api-proof.json`. Passing rows: health/model list, chat, multi-turn chat, Responses, Responses chaining, streaming chat, streaming Responses, parser leak guard, loop guard, token authority, throughput presence, native MTP depth 2 evidence, cache reuse, L2 disk cache, and L2 disk restore. Failed rows: rank agreement reports mismatched ranks `[1,2,3]`, batch throughput is `6.582 tok/s` versus required `40 tok/s`, and hybrid SSM cache proof has SSM hits but no attention evidence. This is no longer stale-ready; it is live but still release-red.
...
- MiniMax #117/#179 public v1.5.56 refresh, 2026-06-06: generated public DMG route/hash contracts for `vMLX-1.5.56-sequoia-arm64.dmg` and `vMLX-1.5.56-tahoe-arm64.dmg` under `build/issue-179/`. Both public DMGs contain the Responses cancel route and engine abort call; latest public and local installed bundled `server.py` hash match at `0be1c30c44da2a57e4a3e89bf87c803d2ae00a86140ff81f7c39e58f5bc3c4f5`. Active audit `build/current-issue179-minimax-k-root-cause-audit-after-reporter-hash-refresh-20260606.json` remains `open` only because reporter-side server hash/model manifest/session/cancel lifecycle metadata is missing and the bad screenshot text is still not reproduced as final visible model output.
...
- MiMo V2.5 stale local cache cleanup, 2026-06-06: removed `/Users/eric/.cache/huggingface/modules/transformers_modules/MiMo_hyphen_V2_dot_5_hyphen_JANG_2L` after the audit identified it as stale local state. Active audit `build/current-mimo-v2-jang2l-current-audit-after-source-preflight-refresh-20260606.json` now reports `stale_local_state_absent=true`, manifest integrity true, source-vs-quant first-divergence proof missing, and release still open for real runtime/model blockers.
...
- MiMo V2.5 JANG_2L source-vs-quant proof requirement, 2026-06-06: the current MiMo audit and release manifest now require `build/current-mimo-v2-jang2l-source-vs-quant-first-divergence-20260606.json` before MiMo release clearance. Source exists on `erics-m5-max2.local` at `/Volumes/EricsLLMDrive/jangq-ai/sources/MiMo-V2.5` (`294G`). Producer script `tests/cross_matrix/run_mimo_v2_source_vs_quant_first_divergence.py` compares already-running source and quant OpenAI-compatible endpoints on the exact MiMo prompt-shape probes and writes that artifact; `--preflight-only --source-host erics-m5-max2.local` records remote source path and missing endpoints as `status=missing_prerequisites` without clearing the gate. Fresh preflight artifact `build/current-mimo-v2-jang2l-source-vs-quant-first-divergence-20260606.json` confirms both source and quant directories exist and the local quant endpoint `127.0.0.1:8897` is healthy; the remaining prerequisite blocker is the missing/unhealthy source endpoint. The passing artifact must be non-remote-only, include source and quant model paths, include non-empty rows, and classify each row as `source_and_quant_match`, `quant_diverges_from_source`, or `source_also_fails` with source and quant outputs. Missing or preflight-only proof emits `mimo_source_vs_quant_first_divergence_missing_or_failed`; this keeps model-artifact versus runtime/decode-loop classification honest.
...
- Qwen3.6 27B MXFP8 TP4 live gateway stall, 2026-06-06: Max2 has an ADLab TP4 gateway listening on `127.0.0.1:8124` for `qwen36-27b-mxfp8-tp4-pod1-shardedvocab`; `/health` and `/v1/models` were reachable over SSH and reported rank targets, cache coordinator, and L2 disk cache enabled. A basic generation/tool/cache probe run locally on Max2 against `/v1/chat/completions` stalled for more than five minutes before only the probe client was terminated; the gateway process was preserved. Local artifact `build/current-qwen36-27b-tp4-live-basic-cache-tool-probe-20260606.json` is `status=open`, classification `gateway_ui_or_adlab_tp4_runtime_stall`, with exact short output, repeat cache hit, tool-call shape, multi-turn recall, and L2 disk hit all unproven. Bounded producer `tests/cross_matrix/run_qwen36_tp4_gateway_probe.py` now runs the gateway request with a hard timeout and captures rank request/response directory snapshots for timeout diagnosis. Current bounded artifact `build/current-qwen36-27b-tp4-gateway-bounded-probe-20260606.json` is `status=open`, classification `gateway_generation_timeout`, with `4` rank targets and `4` rank snapshots captured. This is not a Qwen release pass and needs per-rank request/response/cache diagnostics.
...
- MiMo V2.5 source-vs-quant preflight refresh, 2026-06-06: `build/current-mimo-v2-jang2l-source-vs-quant-first-divergence-20260606.json` now has a healthy local quant endpoint on `127.0.0.1:8897` and a valid Max2 source bundle path at `/Volumes/EricsLLMDrive/jangq-ai/sources/MiMo-V2.5`; the remaining precondition blocker is the missing source endpoint on `erics-m5-max2.local:8126`. A clean Swift TP4 MiMo source relaunch is not currently available without disrupting active Qwen TP4: max2 `8124` is Qwen and rank workers report `TP_MODEL_PATH=/opt/adlab/models/qwen36-27b-mxfp8-mtp`, `TP_SHARDING_PLAN=qwen35`. Pod1 MiMo source materialization also remains incomplete on rank1 (`97` files versus `108` on ranks 0/2/3). MiMo release remains open and model-artifact versus runtime/decode-loop classification remains unresolved until the source rows actually execute.

- MiniMax #179 public v1.5.56 pointer refresh, 2026-06-06: release-manifest/current-suite/public-issue/boundary defaults now point at `build/current-issue179-minimax-k-root-cause-audit-after-reporter-hash-refresh-20260606.json`, `build/current-issue175-179-release-boundary-audit-after-public-v1556-scan-20260606.json`, and `build/current-public-app-issue-audit-after-public-v1556-scan-20260606.json`. Focused `issue179`/public issue regression tests passed (`39 passed`). The release blocker remains open for missing reporter-side installed hash/model/session/cancel lifecycle parity; do not classify it as current public/local route drift.

- MiMo V2.5 source-vs-quant continuation, 2026-06-06: local quant endpoint was relaunched on `127.0.0.1:8897` from `/Users/eric/.mlxstudio/models/JANGQ-AI/MiMo-V2.5-JANG_2L` with conservative simple-engine flags. Health is green, model is MLLM, Metal active memory is about `106GB`, affine quantized matmul is active, and TurboQuant KV telemetry reports enabled. New quant-only exact ACK artifact `build/current-mimo-v2-jang2l-quant-exact-ack-after-source-preflight-refresh-20260606.json` is `status=pass` for short system, long system cache-style, and long no-system ACK rows. This does not clear MiMo: it is narrow quant-side text evidence only.
- MiMo V2.5 source launch blocker, 2026-06-06: source path on Max2 exists at `/Volumes/EricsLLMDrive/jangq-ai/sources/MiMo-V2.5` and Pod1 rank-local source bundles have matching index key sets (`72766` keys each, `79988899808` total_size each). Rank1 has fewer physical safetensor files (`81` vs `93`) but no missing index keys compared with rank0, so file count alone is not the blocker. Source endpoint `erics-m5-max2.local:8126` is still not live. A guarded source launch attempt `codex-mimo-v25-source-8126-activeworker-guard-20260606T185153Z` now fails safely at preflight `rc=78` on all ranks because resident Qwen `TPRankWorker` processes occupy Pod1. Remote AdLab script `~/adlab/scripts/adlab-tp4-resident-serve-launch.sh` was patched in-place so preflight treats any command containing `TPRankWorker` as active; `bash -n` passed. Artifact: `build/current-mimo-v2-jang2l-source-launch-active-worker-guard-20260606.json`. Release classification remains unresolved: source-vs-quant prompt rows cannot run until Qwen workers are intentionally stopped/replaced or another Pod is used.

## 2026-06-06 no-heavy/package pointer refresh after Nemotron C-RADIO bytecode fix

- Fixed `vmlx_engine/omni_multimodal.py` so `_ensure_vendored_cradio_dynamic_module()` ignores and removes `__pycache__` / `.pyc` bytecode when installing the vendored NVIDIA C-RADIO Transformers module into the HF dynamic-module cache. This prevents local `py_compile` / verification imports from poisoning packaged/offline Nemotron Omni media startup and packaged reproducibility checks.
- Refreshed current no-heavy proof pointers away from stale `after-mimo-modality-truth` failures:
  - `build/current-cache-architecture-contract-after-mimo-head-fix-20260606.json` -> `status=pass`.
  - `build/current-parser-registry-contract-during-mimo-sync-20260606.json` -> `status=pass`.
  - `build/current-generation-defaults-contract-during-mimo-sync-20260606.json` -> `status=pass`.
  - `build/current-vl-media-cache-contract-after-bundled-refresh-continuation-20260606.json` -> `status=pass`.
- Rebuilt bundled Python from current source plus local `jang-tools`; `npm run verify-bundled` passed and covers `jang_tools.mimo_v2.mlx_register`, Step3p7 VLM registration, Gemma4 unified registration, TurboQuant kernels, Kimi VLM, and media/audio dependencies.
- Rebuilt and Developer-ID signed the staged Sequoia app at `panel/release/sequoia-app/mac-arm64/vMLX.app`. Electron builder skipped notarization because notarize options were not generated; this is not a public release.
- Refreshed package proof: `build/current-packaged-integrity-contract-after-noheavy-pointer-refresh-20260606.json` -> `status=pass`, including bundled source parity, no packaged `__pycache__`, staged app engine/source parity, hardened runtime/signing preflight, and dry release-gate current-digest usage.
...
- Current root-cause artifact: `build/current-issue179-minimax-k-root-cause-audit-after-reporter-hash-refresh-20260606.json`.
- Reporter installed `server.py` hash from GitHub issue #179 comments is tracked as `a2c4aed88d69b0537de9c7bb7ecb68e4d97f40e92e9fec781e9f79b9b157e8be` with provenance `github_issue_179_comment_jjang-ai_2026-06-02`.
- Route markers are recorded from the published hash context, but the gate remains `open`: no full reporter parity artifact, no reporter model shard/codebook manifest, no reporter session/settings parity, and no reporter cancel lifecycle proof.
- Current classification: blocker stays open; not proven as model artifact corruption, not proven as current public/local runtime route drift, and not release-clearable.

## 2026-06-06 issue179 reporter-hash objective proof refresh

- Latest objective proof: `build/current-objective-proof-after-issue179-reporter-hash-refresh-20260606.json`.
