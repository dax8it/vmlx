...
| Objective proof digest | `build/current-objective-proof-after-n2-jangtq2-l2-live-proof-20260609.json` | status remains open; the N2 row now consumes current-source N2 JANGTQ2 chat/cache/Responses proof and fresh-process L2 restart proof. Proven fields include `stable_text=true`, tool/Responses/stream probes pass, same-process `cache_detail=paged+ssm`, `cached_tokens=8`, restart `cache_detail=paged+ssm+disk`, block-disk hit, and SSM disk hit. N2 still requires JANG_1L runtime/cache/API/UI proof, media/UI/installed-app parity, same-model tunnel parity, and release evidence. |
