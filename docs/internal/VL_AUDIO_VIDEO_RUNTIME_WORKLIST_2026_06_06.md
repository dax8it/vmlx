...
| MiMo JANGTQ2 no-source exactness boundary | `build/current-mimo-v25-jangtq2-exactness-isolation-20260608/result.json` reproduces literal corruption with prefix cache, paged cache, L2, and KV quantization disabled. `build/current-mimo-v25-jangtq2-exactness-no-switchglu-fastpath-diagnostic-20260608/result.json` keeps the same corruption with the MiMo SwitchGLU decode fast path disabled. `build/current-mimo-v25-jangtq2-exactness-no-router-no-switchglu-fastpath-diagnostic-20260608/result.json` keeps the same corruption with both the fallback compiled router and SwitchGLU fast path disabled. `build/current-mimo-v25-jangtq2-tq-kernel-parity-no-source-20260608.json` passes direct-dequant parity on real MiMo layer-1 gate/up/down TQ tensors (`max_abs_diff` around `1e-5` to `2.5e-5`). | This does not prove source-vs-quant first divergence because source load is still intentionally skipped for RAM. It does prove the current exactness failure should not be chased as prefix/paged/L2/KV quant, MiMo SwitchGLU fast path, compiled router, or the shared TQ gather kernel. Next action is either rebuild/requantize JANGTQ2 with a stricter quality contract or authorize a lower-forward/source-vs-quant trace. | `model_artifact` or lower shared forward quality remains unresolved; release stays blocked. |
