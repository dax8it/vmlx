...
| MiMo long-prompt/tool quality | Cache classification and narrow text repeat work; conservative no-cache exact ACK can work. XML-function fallback now respects MiMo's native parser dialect and live `tool_choice=required` emits parsed OpenAI `tool_calls`. The MiMo current audit and release manifest now require a local source-vs-quant first-divergence artifact before release clearance. Producer script `tests/cross_matrix/run_mimo_v2_source_vs_quant_first_divergence.py` compares already-running source and quant endpoints and writes the required artifact. | Run the producer against real source and quant MiMo servers to create `build/current-mimo-v2-jang2l-source-vs-quant-first-divergence-20260606.json`; prove whether long-prompt/exact-cache/speed failures are source-model behavior, quant artifact drift, or runtime decode/cache. Then prove speed target, broader loop behavior, and packaged/UI parity. | `decode_loop` or `model_artifact` still unresolved for quality/speed; parser dialect mismatch fixed in source. |
