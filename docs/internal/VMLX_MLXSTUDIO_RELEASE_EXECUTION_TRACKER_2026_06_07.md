...
| --- | --- | --- |
| Full release checklist | `build/current-full-release-objective-checklist-after-responses-raw-sse-gemma-surface-20260609.json` | `status=open`, `failed_count=120`; consumes current Responses raw-SSE parity, refreshed Step3.7 VLM source-runtime audit, Gemma QAT/native MXFP4 inventory, current MiMo/N2 objective details, and package readiness gates. This is still release-red; no signing, notarization, tagging, or download publishing. |
