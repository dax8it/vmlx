...
| Qwen3.6 27B MXFP8 TP4 | Max2 has an ADLab TP4 gateway on `127.0.0.1:8124` for `qwen36-27b-mxfp8-tp4-pod1-shardedvocab`; health/model metadata showed cache coordinator and L2 disk enabled, but native MTP depth is `0`. Bounded probe producer exists at `tests/cross_matrix/run_qwen36_tp4_gateway_probe.py`; current artifact `build/current-qwen36-27b-tp4-gateway-bounded-probe-20260606.json` captured 4 rank targets and 4 rank snapshots through Max2 as the jump host. | Current corrected classification is `stale_ready_no_rank_workers`: the gateway reports ready from stale `rankN.ready.json` files, the deterministic chat request times out at 15s, and all four ranks have zero actual `TPRankWorker` processes. Exact output, repeat cache hit, tool shape, multiturn recall, streaming, native MTP, and L2 hit are not proven. | Relaunch real resident rank workers or make the gateway fail readiness when only stale ready files exist. After live workers and matched per-rank responses exist, rerun exact output, repeat cache/L2 hit, tools, multi-turn, streaming, and UI-spawned installed-app rows. |
...
| Qwen36 TP4 ADLab gateway response collection | The bounded probe SSHes through Max2, checks `/health` and `/v1/models`, posts a deterministic exact-output chat request, then snapshots every rank request/response directory through the gateway host. The probe now matches recent request IDs to expected per-rank response filenames and records actual `TPRankWorker` process rows from the `comm` field instead of trusting stale ready/response files. | Current artifact has stale ready files but zero live `TPRankWorker` processes, so the gateway's readiness surface is false-positive. Cache coordinator, L2, MTP, and OpenAI endpoint claims cannot be considered live-cleared for this TP4 route. | `gateway_ui` / remote runtime orchestration, not local download corruption. |
...
| MiMo long-prompt/tool quality | Cache classification and narrow text repeat work; conservative no-cache exact ACK can work. XML-function fallback now respects MiMo's native parser dialect and live `tool_choice=required` emits parsed OpenAI `tool_calls`. The MiMo current audit and release manifest now require a source-vs-quant first-divergence artifact before release clearance. Source exists on Max2 at `/Volumes/EricsLLMDrive/jangq-ai/sources/MiMo-V2.5`; producer script `tests/cross_matrix/run_mimo_v2_source_vs_quant_first_divergence.py` compares already-running source and quant endpoints and writes the required artifact; `--preflight-only --source-host erics-m5-max2.local` records remote source path and missing endpoints without clearing the gate. | Run the producer against real source and quant MiMo servers to create `build/current-mimo-v2-jang2l-source-vs-quant-first-divergence-20260606.json`; prove whether long-prompt/exact-cache/speed failures are source-model behavior, quant artifact drift, or runtime decode/cache. Then prove speed target, broader loop behavior, and packaged/UI parity. | `decode_loop` or `model_artifact` still unresolved for quality/speed; parser dialect mismatch fixed in source. |
...
- MiMo text quality still needs source-vs-quant first-divergence tracing and speed proof. The release gate now requires the local source-vs-quant first-divergence artifact before MiMo can clear, and the producer script exists under `tests/cross_matrix/run_mimo_v2_source_vs_quant_first_divergence.py`. The source-side XML-function tool parser/template contract is fixed and live-proven for `record_fact`, but broader loop behavior and packaged/UI parity remain open. Do not force hidden prompts, fake tool-call conversion, or cache disablement as a production fix.
...
- `build/current-mimo-v2-jang2l-current-audit-after-source-vs-quant-required-20260606.json` is the active MiMo audit. It confirms stale MiMo local state is absent after deleting the stale Hugging Face `transformers_modules/MiMo_hyphen_V2_dot_5_hyphen_JANG_2L` cache, while keeping MiMo release open and adding the missing source-vs-quant first-divergence proof as a named blocker.
