...
| Gemma QAT/native MXFP4 expansion | OPEN | Newly tracked release scope: Gemma 3n E2B/E4B QAT/native 4-bit, Gemma4 12B native MXFP4/QAT-style bundles, and Gemma4 26B/31V VL/video bundles if present. Existing Gemma4 JANG_4M image/cache proof does not cover these rows. Required proof includes model-owned generation defaults, parser/tool/reasoning behavior, visual/audio/video where advertised, mixed-SWA/native cache telemetry, TurboQuant KV boundaries where valid, block-disk L2 write and fresh-process restore, Responses streaming content/tool args, UI/CLI settings parity, and installed-app startup parity. |
| Gemma local QAT/native MXFP4 inventory | `build/current-gemma-qat-native-mxfp4-local-inventory-20260609.json` | `status=pass`, `count=11`; local normal roots currently contain Gemma4 12B JANG/MXFP variants, Gemma4 26B JANG_4M CRACK, and Gemma4 31B JANG_4M-MTP. Gemma 3n E2B/E4B QAT/native 4-bit bundles were not present locally, so those rows require download/provided paths before live proof. All present Gemma4 config rows advertise media, so audio/video/VL must either pass live or be capability-gated honestly per bundle. |
