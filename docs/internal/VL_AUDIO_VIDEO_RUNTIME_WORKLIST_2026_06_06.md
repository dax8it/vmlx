...
| Qwen3.6 27B MXFP8 TP4 | Max2 has a live ADLab TP4 gateway on `127.0.0.1:8124` for `qwen36-27b-mxfp8-tp4-pod1-shardedvocab`; health/model metadata showed cache coordinator and L2 disk enabled. Bounded probe producer exists at `tests/cross_matrix/run_qwen36_tp4_gateway_probe.py`; current artifact `build/current-qwen36-27b-tp4-gateway-bounded-probe-20260606.json` captured 4 rank targets and 4 rank snapshots. | Bounded exact-output generation row timed out at 15s (`gateway_generation_timeout`); exact output, repeat cache hit, tool shape, multiturn recall, and L2 hit are not proven. | Inspect captured rank request/response snapshots and cache coordinator state; fix gateway/rank timeout behavior before broader MTP/cache claims. |
