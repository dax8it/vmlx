...
| MiMo no-source exactness classifier | `build/current-mimo-v2-no-source-exactness-classifier-after-audio-expanded-token-l2-restart-20260608.json` | `status=open`; classifies current sentinel failures as `jangtq2_plain_literal_copy_regression_jang2l_plain_copy_passes`. Current evidence proves JANGTQ_2 plain exact literal copy fails before tool parsing or JSON repair can matter, while local JANG_2L preserves plain literals; JANG_2L tool path remains open under tight-memory/Metal OOM pressure. Parser argument rewrite, cache/KV/L2, SwitchGLU fast path, compiled router, and TQ gather kernel are excluded as primary JANGTQ_2 causes; source-vs-quant remains missing. |
