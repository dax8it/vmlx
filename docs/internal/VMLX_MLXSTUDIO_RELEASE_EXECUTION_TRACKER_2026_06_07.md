...
| Responses raw SSE local/gateway/tunnel parity | `build/current-responses-raw-sse-parity-direct-gemma4-e2b-after-parser-20260609.json` | `status=open`, `missing_captures=[gateway,tunnel]`; direct local Gemma4 E2B QAT Responses SSE now emits parse-clean `response.function_call_arguments.delta` (`count=2`) and `response.function_call_arguments.done` (`count=1`) for `record_fact` with authoritative args `{"value": "blue-cat"}` after the Gemma4 parser accepted complete no-end-marker native tool calls. Gateway and tunnel captures with expected args and no reasoning-disabled workaround remain required before this row can pass. |
