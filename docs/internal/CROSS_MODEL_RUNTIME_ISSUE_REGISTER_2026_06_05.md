...
- MiMo V2.5 JANG_2L local artifact is now refreshed from `erics-m5-max2.local:/Users/eric/.mlxstudio/models/JANGQ-AI/MiMo-V2.5-JANG_2L` to `/Users/eric/.mlxstudio/models/JANGQ-AI/MiMo-V2.5-JANG_2L`. The stale local 106G copy was deleted first. Transfer used TB-routed HTTP endpoint `169.254.232.233:8765` after an initial non-TB hostname/rsync attempt; final local manifest proof `build/current-mimo-jang2l-local-manifest-verify-20260606.json` is `status=pass` with 173 files, 113,926,313,468 bytes, 150 top-level safetensor shards, and zero missing/mismatched files.
- MiMo V2.5 JANG_2L runtime/cache bug found and fixed in source on 2026-06-06. Classification: `kernel_cache` runtime issue, not model corruption. Repro artifact `build/current-mimo-jang2l-live-cache-stack-smoke-20260606.json` is `status=review`: first continuous-batching request with paged prefix cache, q8 KV, and block-disk L2 returned `cache ok`, but second identical request hit 25 cached tokens and failed HTTP 500 with `concatenate` mismatch `(1,4,25,192)` vs `(1,8,5,192)`. Root cause: VLM cache extraction normalized every layer to primary `num_key_value_heads=4` even though MiMo SWA `RotatingKVCache` layers legally use `swa_num_key_value_heads=8`. Fix preserves config-declared valid head counts and bumps paged cache schema to `paged_n1_keys_v6`. Post-fix live artifact `build/current-mimo-jang2l-live-cache-stack-after-head-fix-20260606.json` is `status=pass`: request 1 wrote paged/block-L2 cache, request 2 hit 25 cached tokens and returned `cache ok` with HTTP 200. Post-fix cache contract `build/current-cache-architecture-contract-after-mimo-head-fix-20260606.json` is `status=pass`.
- MiMo V2.5 JANG_2L tool behavior is still blocked. Classification: `decode_loop` / `model_artifact` pending source-vs-quant comparison, not a parser crash. Artifact `build/current-mimo-jang2l-live-tool-only-smoke-20260606.json` is `status=review`: source server loaded MiMo in simple MLLM mode with `--enable-auto-tool-choice`, `--tool-call-parser xml_function`, and `--reasoning-parser think_xml`; the forced `lookup_city_code` tool request returned HTTP 200 but emitted raw `<tool_call>` followed by repeated punctuation/fullwidth-comma garbage until `finish_reason=length`, with `tool_calls=null` and zero parsed calls. This proves runtime stays alive and parser does not fabricate tool calls, but MiMo tool-use quality is not release-cleared.
...
- [~] Qwen27 JANG_4M MTP PP proof. Decode clears the lower configured floor, but one PP row is still below target.
