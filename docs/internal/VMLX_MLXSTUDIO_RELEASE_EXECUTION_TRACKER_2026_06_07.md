...
| API/cache/Responses no-heavy contract | `build/current-noheavy-api-cache-contract-after-xml-docs-boundary-20260609.json` | `status=pass`, `missing_markers=[]`; server Responses streaming tool contracts now run 5 regressions, including buffered args, reasoning-channel args, tool-only output index ordering, required empty XML rejection, and preamble + empty XML function fail-closed/no `{}` argument emission. This is source/no-heavy proof, not installed-app or live-family release clearance. |
...
- Current proof artifact: `build/current-noheavy-api-cache-contract-after-xml-docs-boundary-20260609.json`, `status=pass`, `missing_markers=[]`, `responses_streaming_tool_call_arguments_and_indexes=true`, `responses_streaming_tool_contracts rc=0 passed=5`.
...
- Refreshed no-heavy artifact: `build/current-noheavy-api-cache-contract-after-xml-docs-boundary-20260609.json`, `status=pass`, `missing_markers=[]`, `responses_streaming_tool_contracts rc=0 passed=5`.
