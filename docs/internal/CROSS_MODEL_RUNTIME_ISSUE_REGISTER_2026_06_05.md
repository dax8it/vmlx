...
- MiMo V2.5 stale local cache cleanup, 2026-06-06: removed `/Users/eric/.cache/huggingface/modules/transformers_modules/MiMo_hyphen_V2_dot_5_hyphen_JANG_2L` after the audit identified it as stale local state. Active audit `build/current-mimo-v2-jang2l-current-audit-after-first-token-stop-20260606.json` now reports `stale_local_state_absent=true`, manifest integrity true, and release still open for real runtime/model blockers.
