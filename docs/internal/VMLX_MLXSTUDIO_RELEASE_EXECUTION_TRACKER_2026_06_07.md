...
| Objective proof digest | `build/current-objective-proof-after-n2-jangtq2-l2-live-proof-20260609.json` | status remains open; the N2 row now consumes current-source N2 JANGTQ2 chat/cache/Responses proof and fresh-process L2 restart proof. MiMo objective evidence now uses current present artifacts only: structural verify, current audit, lossless-token classifier, and unit-labeled cache-vs-no-cache proof. N2 still requires JANG_1L runtime/cache/API/UI proof, media/UI/installed-app parity, same-model tunnel parity, and release evidence; MiMo still requires exactness/media/decode-speed clearance. |
