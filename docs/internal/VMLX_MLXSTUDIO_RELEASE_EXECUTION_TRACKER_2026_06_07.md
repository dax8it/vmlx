...
| --- | --- | --- |
| Full release checklist | `build/current-full-release-objective-checklist-after-gemma12-code-column-prompt-20260610.json` | `status=open`, `failed_count=57`; consumes current Qwen35 same-model direct/gateway/tunnel Responses raw-SSE parity, Qwen empty-args fail-closed source boundary, refreshed Gemma 12B JANG_4M exact-code proof, Gemma QAT/native MXFP4 inventory, current MiMo/N2 objective details, MiniMax current-source smoke boundary, installed-app parity, and package readiness gates. This is still release-red; no signing, notarization, tagging, or download publishing. |
...
| MiMo JANGTQ2 native TurboQuant contract classifier | `build/current-mimo-v25-jangtq2-native-tq-contract-classifier-20260610.json` | `status=open`; current artifact sidecar tables `codebook.2048.2`, `codebook.4096.2`, `signs.2048.42`, and `signs.4096.42` exactly match the generated `jang_tools` runtime tables, so generated-table use is equivalent to sidecar hydration for this bundle. The main index has 141 prestacked `.tq_packed/.tq_norms/.tq_bits` SwitchMLP groups and no indexed codebook/sign tensors. A direct real-tensor parity check compared native `gather_tq_matmul` against explicit selected-expert dequant reference for MiMo layer 1 gate/down tensors; broadcast, sorted, and per-row shape contracts matched below `1e-6` max absolute diff. Boundary: this narrows the exactness blocker away from sidecar/gather-kernel shape bugs, but MiMo JANGTQ_2 literal exactness remains artifact/logit/requant-profile red. |
...
| Gemma local QAT/native MXFP4 inventory gate | `build/current-gemma-qat-native-mxfp4-local-inventory-after-audio-runtime-gate-20260610.json` | `status=open`, `count=21`, `missing_required_rows=[]`, `source_live_smoke_open_rows=[]`; the no-heavy gate finds local Gemma4 E2B/E4B/12B/26B/31B QAT/native MXFP4 rows and records current-source smoke artifacts for all five required rows. Source-smoke video runtime is now proven for 12B/26B/31B (`vl_blue_video=Blue` plus post-video text recovery `NONE`), so the full checklist no longer fails the three Gemma video-runtime subrows. The release rows still remain `open` until full live proofs exist. Required open rows include `gemma4_e2b_qat_native_mxfp4`, `gemma4_e4b_qat_native_mxfp4`, `gemma4_12b_native_mxfp4`, `gemma4_26b_vl`, and `gemma4_31v_or_31b_vl`. Source smokes are not installed-app/UI/tunnel release clearance; audio/VL must either pass live or be capability-gated honestly per bundle. Current backing split: E2B/E4B have `audio_tower.*` weights and keep audio as runtime-supported; 12B declares audio in config/token metadata but has only `embed_audio.embedding_projection.weight`, so inventory records `audio_declared_by_config=true`, `audio=false`, `audio_runtime_supported=false`, and removes audio from the effective required modality list for 12B rows. 12B image remains weight-backed via `vision_embedder`/`embed_vision`, and 12B/26B/31B video has current-source runtime proof but still lacks installed-app/UI/tunnel parity. |
