...
def _write_current_objective_digest(
    ...
    *,
    ...
    artifact = root / "build/current-objective-proof-after-bundled-refresh-20260606.json"
        ...
        json.dumps(
            {
                ...
                "quant_model_path": "/Users/eric/.mlxstudio/models/JANGQ-AI/MiMo-V2.5-JANG_2L",
                "rows": [
                    {
...
def test_release_regression_manifest_current_sweep_uses_latest_live_smoke_artifacts():
    ...
    assert "current-packaged-integrity-contract-after-bundled-refresh-20260606.json" in joined
...
def test_release_regression_manifest_rejects_stale_issue179_objective_digest_row(
    ...
    _write_current_objective_digest(tmp_path)
    path = tmp_path / "build/current-objective-proof-after-bundled-refresh-20260606.json"
    payload = json.loads(path.read_text(encoding="utf-8"))
...
def test_release_regression_manifest_tracks_tool_calls_with_runner_artifact():
    ...
    assert "current-tool-call-contract-after-xml-function-repair-20260606.json" in joined
...
def test_release_regression_manifest_tracks_packaged_integrity_with_runner_artifact():
    ...
    assert "current-objective-proof-after-bundled-refresh-20260606.json" in joined
...
def test_release_regression_manifest_tracks_current_updater_and_i18n_rechecks():
    ...
    assert "current-objective-proof-after-bundled-refresh-20260606.json" in ling_joined
...
def test_release_regression_manifest_tracks_fresh_dsv4_live_failure_artifact():
    ...
    assert "current-dsv4-route-mode-code-exactness-memory-preflight-cross-family-20260606.json" in joined
