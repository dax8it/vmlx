...
| Objective proof digest | `build/current-objective-proof-after-n2-jangtq2-live-proof-20260609.json` | status remains open; the N2 row now consumes the current-source N2 JANGTQ2 chat/cache/Responses proof (`stable_text=true`, tool/Responses/stream probes pass, `cache_detail=paged+ssm`, `cached_tokens=8`, block-disk writes/hits present, SSM disk stores present). N2 still requires JANG_1L runtime/cache/API/UI proof, media/UI/installed-app parity, and release evidence. |
