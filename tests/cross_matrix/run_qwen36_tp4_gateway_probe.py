...
def _rank_dir_snapshot(host: str, targets: list[dict[str, Any]], *, timeout: float) -> list[dict[str, Any]]:
    ...
    for target in targets:
        ...
        remote_host = str(target.get("host") or "")
        ...
        if remote_host and remote_host != host:
            ...
            result = run_ssh(host, script, timeout=timeout)
        else:
            result = run_ssh(host, rank_script, timeout=timeout)
        snapshots.append({**target, "ssh": result, "snapshot": _parse_last_json_line(result.get("stdout", ""))})
    return snapshots


def rank_snapshots_show_unmatched_responses(result: dict[str, Any]) -> bool:
    ...
    for item in snapshots:
        ...
        if not isinstance(snapshot, dict):
            continue
        unmatched = snapshot.get("unmatched_recent_request_ids")
