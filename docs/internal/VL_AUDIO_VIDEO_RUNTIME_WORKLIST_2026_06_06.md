...
- MiMo text quality still needs source-vs-quant first-divergence tracing and speed proof. The release gate now requires the local source-vs-quant first-divergence artifact before MiMo can clear. The source-side XML-function tool parser/template contract is fixed and live-proven for `record_fact`, but broader loop behavior and packaged/UI parity remain open. Do not force hidden prompts, fake tool-call conversion, or cache disablement as a production fix.
