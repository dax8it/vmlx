...
| --- | --- | --- |
| Full release checklist | `build/current-full-release-objective-checklist-after-jangtq2-live-release-smoke-20260608.json` | `status=open`, `failed_count=18`; consumes the MiMo media runtime stamp-gate audit, no-source exactness classifier rows, Qwen restart/L2/video/long-context checks, and MiniMax #179 refreshed audit. The increase from 16 to 18 is intentional release honesty: current MiMo bundles fail closed as text runtime instead of over-advertising media support. |
...
| Gemma QAT/native MXFP4 12B/26B/31B audio capability boundary | `build/current-all-local-model-smoke-gemma4-12b-qat-mxfp4-fullmedia-tools-l2-after-tool-result-quoted-target-20260609/summary.json` | 12B QAT source smoke is now `status=pass`, `failures=0` for text, image, video, tools, cache, and L2. No audio probe is emitted by the current runner because runtime capabilities classify the bundle as vision modality. Index audit shows E2B/E4B have full audio tower weights, 26B/31B have zero audio weights, and 12B has only `embed_audio.embedding_projection.weight`; audio must be capability-gated honestly per bundle instead of inferred from token metadata alone. 26B/31B remain open until rerun with the quoted tool-result harness and memory-safe media/cache settings. |
| Gemma local QAT/native MXFP4 inventory gate | `build/current-gemma-qat-native-mxfp4-local-inventory-20260609.json` | `status=open`, `count=16`; the no-heavy gate now finds local Gemma4 E2B/E4B/12B/26B/31B QAT/native MXFP4 rows and keeps them `open` until live proofs exist. Required open rows include `gemma4_e2b_qat_native_mxfp4`, `gemma4_e4b_qat_native_mxfp4`, `gemma4_12b_native_mxfp4`, `gemma4_26b_vl`, and `gemma4_31v_or_31b_vl`. All present Gemma4 config rows advertise media, so audio/video/VL must either pass live or be capability-gated honestly per bundle. |
...
| Responses raw SSE local/gateway/tunnel parity | `build/current-responses-raw-sse-parity-20260609.json` | `status=open`, `missing_captures=[direct,gateway,tunnel]`; classifier now requires raw direct local server, panel gateway, and tunnel SSE captures with clean parse, matching expected function `lookup`, matching expected authoritative arguments `{"query":"alpha"}`, and reasoning-summary SSE events present. This prevents source/gateway unit coverage or a reasoning-disabled workaround from being mistaken for live tunnel parity. |
...
- Tightened 2026-06-09 follow-up: the default current-suite command now requires expected function `lookup`, expected authoritative arguments `{"query":"alpha"}`, clean SSE JSON parsing, and reasoning-summary SSE events. This specifically guards the no-reasoning-disable workaround boundary.
- Current artifact: `build/current-responses-raw-sse-parity-20260609.json`, `status=open`, `missing_captures=[direct,gateway,tunnel]`, `expected.require_reasoning_events=true`. This is expected until raw captures are supplied; it must not be treated as pass.
- Boundary: this is a capture classifier and proof-map pin, not a live tunnel proof. A passing row still requires all three raw SSE captures with matching expected function-call arguments and reasoning events.
