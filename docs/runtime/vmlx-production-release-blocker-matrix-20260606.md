...
Current status: red. MiMo required tools now pass in the current no-media source
gate, but tool-result continuation, auto-tool behavior, loop-stop behavior, raw
markup leakage, and Step/LFM/MiniMax/Nemo parity remain incomplete.
