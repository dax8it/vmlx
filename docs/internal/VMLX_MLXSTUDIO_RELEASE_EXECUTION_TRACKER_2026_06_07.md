...
| MiMo no-source exactness classifier | `build/current-mimo-v2-no-source-exactness-classifier-after-audio-expanded-token-l2-restart-20260608.json` | `status=open`; classifies current sentinel failures as `jangtq2_plain_literal_copy_fails_before_parser_or_json_repair`. Current evidence proves plain exact literal copy fails before tool parsing or JSON repair can matter, and excludes parser argument rewrite, cache/KV/L2, SwitchGLU fast path, compiled router, and TQ gather kernel as primary cause; source-vs-quant remains missing. |
...
| `mimo_jangtq2_artifact_exactness_blocked` | plain literal copy fails before parser/JSON repair | Required plain text, tool args, and JSON exact values must preserve literals such as `blue-cat` and `B7-CAT-09`; JSON repair or parser rewriting must not rewrite semantic values to fake a pass. |
